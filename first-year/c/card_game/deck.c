...
struct card *PopCardInDeck(struct deck *d) {
    struct elem* e = d->firstElem;
    d->firstElem = e->nextElem;
    return e->elem;
}

void mixCards(struct deck *d) {
    int nbCards = CountCardInDeck(d);
    for(int i = 0; i < nbCards; i++){
        int random = rand() % nbCards;
        struct card* c = GetCardInDeckAt(d, i);
        struct card* c2 = GetCardInDeckAt(d, random);
        struct card* c3 = c;
    }
}
